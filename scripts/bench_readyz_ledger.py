...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='xray-ledger-') as directory:
        ...
        for i in range(1, args.events+1):
            ...
            events_batch.append((eid, 'synthetic', 'test', 'fixture', 'synthetic', '', previous, head, stamp))
            ckpt_batch.append((eid, i, head, audit.checkpoint_signature(eid, i, head, server.AUDIT_KEY), stamp))
            previous = head
            if len(events_batch) >= 5000:
                c.executemany('INSERT INTO audit_events(event_id,actor,action,object_type,object_id,detail,previous_hash,event_hash,created_at) VALUES(?,?,?,?,?,?,?,?,?)', events_batch)
                c.executemany('INSERT INTO audit_checkpoints(event_id,event_count,head_hash,signature,created_at) VALUES(?,?,?,?,?)', ckpt_batch)
                c.commit()
                events_batch.clear()
                ckpt_batch.clear()
        if events_batch:
        ...
        while True:
            attempts += 1
            try:
                result = server.readiness_verify_audit(c)
                break
        ...
        out = dict(scope='local synthetic SQLite readiness helper, fixed IDs and clock, no network', events=args.events,
            verified_events=result['events'], verification_attempts=attempts, cold_ms=round(cold,3), warm_p50_ms=round(statistics.median(warm),3),
